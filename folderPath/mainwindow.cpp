        ...
        if (onlyCurrent.size() > 0) {
            ...
            foreach (const QString &oC, onlyCurrent) {
                ...
                if (fileType == ".pcap") {
                    ...
                    connect(thread,&QThread::started,pw,&PacketWorker::createPacket);
                    connect(pw,&PacketWorker::createFinished,thread,&QThread::quit);
                    connect(pw,&PacketWorker::createFinished,pw,&PacketWorker::deleteLater);
...
void mainwindow::parsePcapFile(){
    ...
    if (dialog.exec() == QDialog::Accepted) {
        ...
        connect(qt,&QThread::started,worker,&PacketWorker::createPacket);
        connect(worker,&PacketWorker::createFinished,qt,&QThread::quit);
        connect(worker,&PacketWorker::createFinished,worker,&PacketWorker::deleteLater);
